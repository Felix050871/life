...
## Recent Changes
- July 30, 2025: **Sistema Overlay Loading Ottimizzato per Export** - Ripristinato sistema overlay globale in base.html ma ottimizzato specificamente per operazioni export: overlay si attiva solo su pulsanti export specifici (non su tutti i form), timeout ridotto a 10 secondi con auto-hide dopo 3 secondi per download, rimossi event listeners automatici su navigazione, multipli listener per pagehide/visibilitychange per prevenire persistenza, sistema ora funziona correttamente per export mantenendo feedback UX senza bloccare interfaccia
