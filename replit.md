...
## Recent Changes
- July 30, 2025: **Dashboard Team Range Date Personalizzato Completato** - Implementata selezione range date personalizzato nella dashboard team: pulsante "Range" con modal interattivo per selezione date inizio/fine, logica backend per gestione period='custom' con parsing parametri start_date/end_date, navigazione temporale mantenuta anche per range personalizzati, validazione date con fallback automatico, cambiate tutte le diciture "Oggi" in "Giorno" per coerenza interfaccia, sistema completo per analisi presenze su periodi personalizzati
