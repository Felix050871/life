...
- **Social Safety Net System (Ammortizzatori Sociali)**: Comprehensive system for managing Italian workforce reduction programs (CIGS, Solidarity Funds, FIS, etc.) with multi-tenant isolation. Features include: program creation with legal decree tracking and document upload, employee assignments with effective date ranges and approval workflow, automatic snapshot integration with Contract History system, automatic weekly hour reduction enforcement via `get_active_work_hours_week()` applying program-specific percentage reductions, two-step validation flow (enforce_safety_net_constraints → validate_weekly_limit) preventing overtime violations and ensuring compliance with reduced hour caps, AttendanceEvent enhancement with `safety_net_assignment_id` and `payroll_code` fields for complete data lineage, integration with both manual timesheet entry and bulk autofill systems (bulk fill skips non-compliant days with warnings), permission-based access control (can_manage_social_safety_programs, can_assign_social_safety_programs, can_view_social_safety_reports), and centralized validation utilities for program-specific constraints (overtime prohibition enforcement). Database models include SocialSafetyNetProgram (company-level with INPS codes and reduction percentages) and SocialSafetyNetAssignment (employee-level with temporal validity and approval tracking). System designed for future extensibility with leave accrual adjustments, payroll export integration with special INPS codes, HR detail timeline views, and dashboard monitoring widgets.
- **CCNL Structured Data System**: Hierarchical three-tier management system for Italian national collective labor agreements (CCNL → Qualification → Level) replacing legacy free-text fields with relational data. Features include: three database models (CCNLContract, CCNLQualification, CCNLLevel) with multi-tenant isolation and cascade deletes, dual-field migration strategy maintaining legacy string fields (ccnl, qualifica, ccnl_level) alongside new FK references (ccnl_contract_id, ccnl_qualification_id, ccnl_level_id) for backward compatibility during phased rollout, idempotent seed script (`scripts/seed_ccnl_data.py`) populating standard Italian contracts (Commercio: 9 qualifications/56 levels, Metalmeccanici: 9/56, Terziario: 9/18), dedicated blueprint (`blueprints/ccnl.py`) with full CRUD for all three hierarchy levels plus JSON API endpoints (/api/ccnl/<id>/qualifications, /api/qualifications/<id>/levels) for cascading dropdowns in HR forms, permission-based access control (can_manage_ccnl, can_view_ccnl) enforced via decorators, unique constraints preventing duplicates within parent scope ((nome, company_id) for CCNL, (nome, ccnl_id) for qualifications, (codice, qualification_id) for levels), and admin UI allowing company-specific overrides of standard contracts. Migration executed via `migrations/add_ccnl_structured_data.sql`. System designed for gradual adoption with feature flag support (planned), enabling canary rollout per tenant and eventual deprecation of legacy string fields after ≥95% FK coverage sustained for 60 days.
