...
**Attendance Types System**: Implemented comprehensive configurable attendance types system. Created `AttendanceType` model with fields: name, description, active status, is_default flag, company_id for multi-tenant isolation. Added `attendance_type_id` field to `AttendanceEvent` model with foreign key relationship. Complete CRUD management interface with permissions (can_manage_attendance_types, can_view_attendance_types). Integrated into manual attendance UI with dropdown selection for each day. Populated three default types for all companies: "Ordinario" (default), "Trasferta nazionale", "Trasferta internazionale". Multi-tenant security enforced: attendance_type_id validated using filter_by_company() and active status check in save operations. UI captures attendance type from any event type (clock_in, clock_out, breaks) to prevent data loss on partial-day entries. Utility script `populate_attendance_types.py` for initial data seeding.

**Previous Update (October 27, 2025)**: Implemented comprehensive timesheet consolidation system with reopen request approval workflow. Once a monthly timesheet is consolidated (MonthlyTimesheet.is_consolidated), employees cannot insert, edit, or delete attendance entries for that period. Employees must submit a TimesheetReopenRequest with a reason, which requires approval from authorized personnel (HR managers, project managers with commesse permissions, or admins). Upon approval, the timesheet is reopened (is_consolidated set to False) allowing modifications. All manual entry routes (manual_entry, edit_manual_entry, delete_manual_entry) validate consolidation status before allowing operations. UI includes alert banner on attendance page when consolidated, modal for submitting reopen requests, and dedicated management page for reviewers. Multi-tenant filtering enforced on all reopen request queries. Status values follow capitalized convention: 'Pending', 'Approved', 'Rejected'.
