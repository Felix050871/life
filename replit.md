...
- **Work Schedule Management** (October 24, 2025): Flexible work schedule system decoupled from sede. Work schedules can be configured as **global** (company-wide, available to all employees) or **sede-specific** (limited to specific locations). Each user can be assigned a work schedule regardless of their operational sede. Database schema: `WorkSchedule.sede_id` is nullable; unique constraint changed from `(sede_id, name)` to `(company_id, name)`.
