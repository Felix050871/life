...
def index():
    ...
    tools = filter_by_company(
        ...
        current_user
    ).order_by(asc(HublyToolLink.sort_order), asc(HublyToolLink.name)).all()
    ...
    return render_template('hubly/tools/index.html', tools_by_category=tools_by_category)
...
def manage():
    ...
    tools = filter_by_company(
        HublyToolLink.query,
        current_user
    ).order_by(asc(HublyToolLink.sort_order)).all()
    
    return render_template('hubly/tools/manage.html', tools=tools)
...
def create():
    ...
    return render_template('hubly/tools/create.html')
...
def edit(tool_id):
    ...
    tool = filter_by_company(HublyToolLink.query, current_user).get_or_404(tool_id)
    ...
    return render_template('hubly/tools/edit.html', tool=tool)
