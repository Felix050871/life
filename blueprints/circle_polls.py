...
bp = Blueprint('circle_polls', __name__, url_prefix='/circle/polls')
...
def view(poll_id):
    ...
    poll = filter_by_company(CirclePoll.query, current_user).filter_by(id=poll_id).first_or_404()
    ...
    return render_template('circle/polls/view.html', 
                         poll=poll, 
                         results=results,
                         has_voted=has_voted,
...
def vote(poll_id):
    ...
    for option_id in option_ids:
        option = CirclePollOption.query.filter_by(id=option_id, poll_id=poll_id).first()
        if option:
            vote = CirclePollVote(
                poll_id=poll_id,
...
def delete(poll_id):
    ...
    # Elimina voti
    CirclePollVote.query.filter_by(poll_id=poll_id).delete()
    
    # Elimina opzioni
    CirclePollOption.query.filter_by(poll_id=poll_id).delete()
