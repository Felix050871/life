...
based on employee HR data and approved leave requests.
"""
...
from models import User, UserHRData, LeaveRequest, db
...
def calculate_leave_balance(user_id: int, reference_date: Optional[date] = None) -> Dict[str, Decimal]:
    """
    Calculate leave (ferie) balance for a user.
    ...
    Returns:
        Dictionary with:
            - accrued_days: Total days accrued based on months worked
            - used_days: Total days used (approved leave requests)
            - balance_days: Remaining balance (accrued - used)
            - monthly_accrual: Days accrued per month
    ...
    months_worked = calculate_months_worked(hr_data.hire_date, reference_date)
    
    # Calculate accrued days
    accrued_days = monthly_accrual * months_worked
    
    # Apply part-time percentage if applicable
    if hr_data.part_time_percentage and hr_data.part_time_percentage > 0:
        ...
        accrued_days = accrued_days * part_time_factor
...
def calculate_permit_balance(user_id: int, reference_date: Optional[date] = None) -> Dict[str, Decimal]:
    """
    Calculate permit (permessi/ROL) balance for a user.
    ...
    accrued_hours = monthly_accrual * months_worked
    
    # Apply part-time percentage if applicable
    if hr_data.part_time_percentage and hr_data.part_time_percentage > 0:
        part_time_factor = Decimal(str(hr_data.part_time_percentage)) / Decimal('100')
        accrued_hours = accrued_hours * part_time_factor
...
def get_all_leave_balances(company_id: int, reference_date: Optional[date] = None) -> list:
    ...
    """
    # Get all active users in the company with HR data
