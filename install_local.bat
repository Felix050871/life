...
REM =============================================================================
REM DATI DI TEST (OPZIONALE)
REM =============================================================================

echo [INFO] 8. Dati di test (opzionale)...

set /p LOAD_TEST_DATA="Vuoi caricare dati di test nel database? (y/N): "
if /i "!LOAD_TEST_DATA!"=="y" (
    echo [INFO] Caricamento dati di test...
    if exist "populate_test_data.py" (
        "%VENV_DIR%\Scripts\python.exe" populate_test_data.py
        if %errorlevel% equ 0 (
            echo [OK] Dati di test caricati con successo
        ) else (
            echo [WARN] Errore durante il caricamento dati di test
        )
    ) else (
        echo [WARN] File populate_test_data.py non trovato
    )
) else (
    echo [INFO] Dati di test saltati
)
